...
namespace arm_trajectory_action{
    ...
    ArmTrajectoryAction::ArmTrajectoryAction(const rclcpp::NodeOptions &options):Node("traj_service", options) {
        using namespace std::placeholders;
        auto bind_callback = std::bind(&ArmTrajectoryAction::srv_callback, this, _1, _2);
        arm_trajectory_service = this->create_service<arm_traj_srv>("arm_trajectory_service", bind_callback);
    }

    void ArmTrajectoryAction::srv_callback(const std::shared_ptr<arm_traj_srv::Request> request,
